...
#
# Usage:
#   scripts/check-deps.sh              # check only, report what's missing
#   scripts/check-deps.sh --install    # also install what's missing
#   scripts/check-deps.sh --yes        # assume yes to install prompts
#
# Wired up as:
#   npm install         -> runs this in check-only mode (postinstall hook)
...
# `npm install` runs this on every install — in CI, in `docker build`, with
# closed/piped stdin — so by default installing is only attempted when a
# human is plausibly watching an interactive `npm install`. `--install`
# (what `npm run setup:deps` passes) is an explicit request and always runs
# regardless of TTY/CI/Docker; `--check` always forces report-only.
# Individual sudo/elevation steps get their own confirm() prompt regardless.
if [[ "$FORCE_CHECK" == "1" ]]; then
  INSTALL=0
elif [[ "$FORCE_INSTALL" == "1" ]]; then
  INSTALL=1
elif [[ -n "${CI:-}" ]] || [[ -f /.dockerenv ]] || ! is_tty; then
  INSTALL=0
else
  INSTALL=1
fi
